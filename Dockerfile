...
RUN mkdir -p /diskmark/usr/bin /diskmark/lib /diskmark/usr/lib /diskmark/etc /diskmark/disk && \
    for bin in bash cat cut dd df grep ls mkdir rm sed awk basename dirname env expr fio findmnt free head lsblk numfmt perl printf rev ssl_client tail tput tr wc wget; do cp $(which $bin) /diskmark/usr/bin/; done && \
